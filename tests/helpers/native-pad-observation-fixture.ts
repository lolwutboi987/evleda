...
export async function nativePadObservationFixture(source:string,libraryBaseline=source){
  ...
  const layer=(value:string)=>value==="*.Cu"?[...copper,...Array.from({length:30},(_,i)=>`BL_In${i+1}_Cu`)]:[`BL_${value.replace(".","_")}`];
  const rawPads=board.footprints.flatMap(fp=>fp.pads.map(p=>({id:{value:p.physical.id!},...(p.number?{number:p.number}:{}),net:p.netName?{name:p.netName}:{},
    type:p.physical.padType==="thru_hole"?"PT_PTH":"PT_SMD",position:{x_nm:String(Math.round(p.at.x*1e6)),y_nm:String(Math.round(p.at.y*1e6))},
    pad_stack:{type:"PST_NORMAL",layers:p.layers.flatMap(layer),angle:{value_degrees:p.physical.rotationDeg},
      drill:{start_layer:"BL_F_Cu",end_layer:"BL_B_Cu",diameter:p.physical.drill?{x_nm:String(Math.round(p.physical.drill.sizeMm.x*1e6)),y_nm:String(Math.round(p.physical.drill.sizeMm.y*1e6))}:{},shape:"DS_CIRCLE"},
      copper_layers:[{layer:"BL_F_Cu",shape:p.physical.shape==="circle"?"PSS_CIRCLE":p.physical.shape==="roundrect"?"PSS_ROUNDRECT":"PSS_RECTANGLE",
  ...
  const requested=rawPads.filter(p=>p.number&&p.net.name).map(p=>p.id.value);
  const payload={schemaVersion:"evleda.kicad-live-pcb-pad-snapshot.v1",documentBefore:document,documentAfter:document,boardSourceBefore:source,boardSourceAfter:source,
    ...
    padstackPresence:{requestType:"kiapi.board.commands.CheckPadstackPresenceOnLayers",request:{board:document,items:rawPads.map(p=>p.id),layers:copper},responseType:"kiapi.board.commands.PadstackPresenceResponse",
      response:{entries:rawPads.flatMap(p=>copper.map(layer=>({item:p.id,layer,presence:p.pad_stack.layers.includes(layer)?"PSP_PRESENT":"PSP_NOT_PRESENT"})))}},
    connectivity:requested.map(sourcePrimitiveId=>({sourcePrimitiveId,requestType:"kiapi.board.commands.GetConnectedItems",request:{header:{document},items:[{value:sourcePrimitiveId}],types:["KOT_PCB_PAD"]},responseType:"kiapi.common.commands.GetItemsResponse",responseMetadata:{status:"IRS_OK"},
      padRecordIndexes:rawPads.flatMap((p,i)=>p.net.name&&p.net.name===rawPads[byId.get(sourcePrimitiveId)!]!.net.name?[i]:[])}))};
  const inspections=baseline.footprints.map(fp=>{
    const pads=fp.pads.map((p,ordinal)=>({ordinal,number:p.number,padType:p.physical.padType!,shape:p.physical.shape!,layers:p.layers,copperSides:["front" as const],role:p.number?"numbered-copper" as const:"paste-aperture" as const,

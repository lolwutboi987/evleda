...
export function createKicadToolboxWorkspace(options: KicadToolboxWorkspaceOptions) {
  ...
  for (const revisionKind of ["placement", "via-budgets", "plane-regions"] as const) toolbox.server.registerTool(
    revisionKind === "placement" ? "evleda_revise_placement" : revisionKind === "via-budgets" ? "evleda_revise_via_budgets" : "evleda_revise_plane_regions", {
    ...
    if (existing !== undefined) {
      const expectedLineage = revisionKind === "placement" ? "evleda.plane-placement-revision-lineage.v1"
        : revisionKind === "via-budgets" ? "evleda.plane-via-budget-revision-lineage.v1" : "evleda.plane-region-policy-revision-lineage.v1";

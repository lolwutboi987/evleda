...
function drillTopology(value: FreshPlaneAcceptanceAssessment["planes"][number]["drillTopology"]) {
  ...
  ...(value.regionalInteriors === undefined ? {} : { regionalInteriors: {
    status: value.regionalInteriors.status, issues: reasons(value.regionalInteriors.issues),
    observedComponentCount: value.regionalInteriors.observedComponentCount,
    components: value.regionalInteriors.components.map(component => ({
      nativePolygonIndex: component.nativePolygonIndex, status: component.status, issues: reasons(component.issues),
      planarInteriorConnected: component.planarInteriorConnected, cachedAreaTwiceNm2: component.cachedAreaTwiceNm2,
      conservativeAreaLowerBoundTwiceNm2: component.conservativeAreaLowerBoundTwiceNm2,
      classificationComplete: component.classificationComplete,
      bores: component.bores.map(bore => ({ uuid: publicText(bore.uuid), classification: bore.classification,
        classificationBasis: bore.classificationBasis, issues: reasons(bore.issues) })),
    })),
    interRegionConnectivity: value.regionalInteriors.interRegionConnectivity,
    maximumComponents: value.regionalInteriors.maximumComponents, maximumBoreRelations: value.regionalInteriors.maximumBoreRelations,
  } }),

...
export const DEFAULT_PCB_HARNESS_MUTATION_TOOL_NAMES = Object.freeze([
  "fresh_set_footprint_fields", "fresh_set_footprint_poses", "pcb_add_text", "fresh_set_schematic_field_positions", "fresh_set_schematic_symbol_poses",
  ...
  for (let iteration = 1; iteration <= iterationLimit; iteration += 1) {
    ...
    if (turn.stopReason === "tool_calls") {
      ...
      const needsSchematicReadback = readbackTool !== undefined && turn.toolCalls.some((call) =>
        ["sch_apply_plan", "sch_add_symbol", "sch_modify_property", "fresh_apply_contract_connectivity", "fresh_apply_recommended_schematic_placement", "fresh_autoplace_schematic_fields", "fresh_set_schematic_field_positions", "fresh_set_schematic_symbol_poses", "sch_move_symbol", "lib_assign_footprint"].includes(call.name),
      );
    ...
    const reachedPhaseBoundary = turn.stopReason === "completed"
      ...
      || turn.toolCalls.some((call) => call.name === "fresh_set_schematic_symbol_poses" || call.name === "fresh_set_schematic_field_positions" || call.name === "fresh_set_footprint_fields" || call.name === "fresh_set_footprint_poses" || call.name === "fresh_autoplace_schematic_fields" || call.name === "pcb_sync_from_schematic" || call.name === "fresh_sync_from_schematic" || call.name === "fresh_replace_route_items" || (mutationToolNames.has(call.name) && call.name.startsWith("pcb_")));

...
export const DEFAULT_PCB_HARNESS_MUTATION_TOOL_NAMES = Object.freeze([
  "fresh_set_footprint_fields", "pcb_add_text", "fresh_set_schematic_field_positions", "fresh_set_schematic_symbol_poses",
...
export async function runPcbAgentHarness(
  ...
  for (let iteration = 1; iteration <= iterationLimit; iteration += 1) {
    ...
    const reachedPhaseBoundary = turn.stopReason === "completed"
      ...
      || turn.toolCalls.some((call) => call.name === "fresh_set_schematic_symbol_poses" || call.name === "fresh_set_schematic_field_positions" || call.name === "fresh_set_footprint_fields" || call.name === "fresh_autoplace_schematic_fields" || call.name === "pcb_sync_from_schematic" || call.name === "fresh_sync_from_schematic" || call.name === "fresh_replace_route_items" || (mutationToolNames.has(call.name) && call.name.startsWith("pcb_")));

...
export function getPcbPlaneDesignIntentModelGuide(includeInterfaceRequirements = false, includeExternalPowerInputs = false, includeChannel = false, includeDerivedPowerSources = false): string {
  return PCB_PLANE_DESIGN_INTENT_MODEL_GUIDE + (includeInterfaceRequirements ? `\n${PCB_PLANE_INTERFACE_REQUIREMENTS_MODEL_GUIDE}` : "")
    + (includeExternalPowerInputs ? `\n${PCB_PLANE_EXTERNAL_POWER_MODEL_GUIDE}` : "") + (includeChannel ? `\n${PCB_PLANE_CHANNEL_MODEL_GUIDE}` : "")

...
export const PCB_PLANE_DESIGN_INTENT_MODEL_GUIDE = [
  ...
  "Each plane-topology route has exactly net, topology:\"plane\", planeId, accessRouting; no top-level preferredLayer, maxVias, routeLength or referencePath. planeId/accessRouting may be null; non-null accessRouting requires nullable preferredLayer, maxVias, routeLength. Access preferredLayer is F.Cu, B.Cu or \"either\" (both class layers required). Tracks name one layer; plane.layer stays exact. No implicit transitions. Each plane has one owner matching net/planeId. Access limits cover tracks/vias, not plane interior.",
...
export const PCB_PLANE_DESIGN_INTENT_EXTENDED_MODEL_GUIDE_MAX_UTF8_BYTES = 20 * 1024;

...
The [four-layer native workflow](docs/four-layer-construction.md) now creates, routes and fills a separate synthetic four-layer project. Host37/DOC16 verifies both internal planes independently and checks through-via membership on all enabled layers. The fixture has three connected functional nets, configured ERC/DRC zero, and verified source/native plane inventory; it remains unaccepted because reference/interface and physical continuity requirements remain unresolved. The focused suite passed 471 tests, source/UI type checking and backend build passed, and the complete 8469-file DOC16 runtime verified. These results qualify tooling, not the RP2350. The RP2350 managed board remains 60-13 with 14 disconnected nets. **Return to actual RP2350 completion; avoid further optional tooling expansion.**

The user permits considering four layers. The [four-layer feasibility study](designs/rp2350-pico/four-layer-feasibility-20260919/README.md) gives a concrete reason to pursue signal/ground/ground/signal: two internal ground regions plus eight own-header inner leads connect 63/64 ground pads without new vias or reported clearance/short/hole-clearance findings. The study remains unadopted and its retained two-layer physical-stackup declaration is explicitly rejected by the reader. Native four-layer creation is now available; placement revision still cannot change construction. Use a supported new project with a defensible declared stackup, preserve the old project, and finish actual placement/routing. Host30/DOC14 and the managed 60-13 board remain unchanged.

...
The [four-layer construction and compiler](docs/four-layer-construction.md) now support explicit four-layer drafts, two named ground planes under one routing net, authenticated construction generation, and saved/public observations of all copper layers and three dielectric gaps. Front/back model inputs use the appropriate adjacent gap. Old two-layer contract/bundle/DRU golden identities are unchanged. **Four-layer native authoring remains unavailable:** creation rejects before allocation/preparation until internal-layer operations, two-plane evidence and a new runtime are qualified. Final affected-scope runs passed 46 project/workspace tests and 63 construction/report/interface tests; source/UI type checking and isolated backend build passed. The RP2350 managed board remains 60-13.

...
def main() -> None:
    ...
    pad_addon_path = os.path.join(os.path.dirname(os.path.realpath(__file__)), "evleda_live_pcb_pad_snapshot.py")
    pad_spec = importlib.util.spec_from_file_location("evleda_live_pcb_pad_snapshot", pad_addon_path)
    if pad_spec is None or pad_spec.loader is None:
        _fail()
    pad_addon = importlib.util.module_from_spec(pad_spec)
    sys.modules[pad_spec.name] = pad_addon
    pad_spec.loader.exec_module(pad_addon)
    pad_addon.install(upstream_server)

    batch_addon_path = os.path.join(os.path.dirname(os.path.realpath(__file__)), "evleda_schematic_connectivity_batch.py")
    batch_spec = importlib.util.spec_from_file_location("evleda_schematic_connectivity_batch", batch_addon_path)
    if batch_spec is None or batch_spec.loader is None:
        _fail()
    batch_addon = importlib.util.module_from_spec(batch_spec)
    sys.modules[batch_spec.name] = batch_addon
    batch_spec.loader.exec_module(batch_addon)
    batch_addon.install(upstream_server)

    plane_package = os.path.join(os.path.dirname(os.path.realpath(__file__)), "evleda_plane_stage")
    plane_spec = importlib.util.spec_from_file_location(
        "evleda_plane_stage", os.path.join(plane_package, "__init__.py"),
        submodule_search_locations=[plane_package],
    )
    if plane_spec is None or plane_spec.loader is None:
        _fail()
    plane_addon = importlib.util.module_from_spec(plane_spec)
    sys.modules[plane_spec.name] = plane_addon
    plane_spec.loader.exec_module(plane_addon)
    plane_addon.install(upstream_server)

...
# Encoding selector only. All native observations remain complete in structuredContent.
COMPACT_TEXT = '{"schemaVersion":"evleda.kicad-live-pcb-pad-snapshot-envelope.v2","payloadLocation":"structuredContent","payloadSchemaVersion":"evleda.kicad-live-pcb-pad-snapshot.v1"}'
...
def _snapshot_result(payload: dict[str, Any]) -> CallToolResult:
    # Retain strict JSON validation; Pydantic alone can normalize non-finite numbers.
    json.dumps(payload, ensure_ascii=False, separators=(",", ":"), allow_nan=False)
    result = CallToolResult(content=[TextContent(type="text", text=COMPACT_TEXT)], structuredContent=payload, isError=False)
...
def read_live_pcb_pad_snapshot(requested_primitive_ids: list[str], client_getter: Callable[[], Any] = _owned_client) -> CallToolResult:
    ...
    try:
        ...
        result = _snapshot_result(payload)
        _deadline(deadline)
        return result
    except ToolError:
        raise

...
from ..utils.field_layout import (
    Symbol as PresentationSymbol,
    body_and_pins as _presentation_body_and_pins,
    fields as _presentation_fields,
    find_symbol_span as _find_presentation_symbol_span,
    obstacles_for as _presentation_obstacles,
    plan_fields as _plan_presentation_fields,
    presentation_model as _presentation_model,
    presentation_signature as _presentation_signature,
    set_field_presentation as _set_field_presentation,
)
...
def rotate_point(x: float, y: float, angle_deg: float) -> tuple[float, float]:
    """Rotate an already Y-inverted library offset into schematic sheet space.

    KiCad applies placement rotation before library-to-sheet Y inversion;
    callers have already inverted Y, so the remaining rotation is negative.
    """
...
def place_symbol_block(
    ...
    if not is_power_symbol:
        # Initial fields must clear the actual local body/pin corridor. The
        # explicit sch_autoplace_fields operation later reserves sheet obstacles.
        baseline = result
        candidate = baseline
        try:
            library_name, symbol_name = _split_lib_id(lib_id)
            library_block = load_lib_symbol(library_name, symbol_name)
            if library_block is None:
                raise ValueError("No local symbol definition for initial field placement")
            body, pins = _presentation_body_and_pins(library_block, lib_id, unit, x, y, rotation)
            target = PresentationSymbol(reference, lib_id, unit, x, y, rotation, body, pins,
                                        _presentation_fields(baseline))
            specs, placements = _plan_presentation_fields(target, [body.expanded(0.762)])
            for spec, placement in zip(specs, placements, strict=True):
                candidate = _set_presentation_field(candidate, spec.name, placement.x, placement.y,
                                                    placement.angle, placement.justify)
            if _presentation_signature(baseline) != _presentation_signature(candidate):
                raise ValueError("Initial field placement changed non-presentation structure")
            result = candidate
        except (OSError, ValueError) as exc:
            # Presentation coverage must not reject an otherwise supported
            # symbol. Retain legacy initial fields and expose UNKNOWN in logs;
            # only the explicit repair operation refuses incomplete coverage.
            result = baseline
            logger.warning("initial_symbol_field_layout_unknown", lib_id=lib_id,
                           reference=reference, error=str(exc), native_readability_verified=False)
    return result
    ...
    *,
    ...
    with _SCHEMATIC_WRITE_LOCK:
        ...
        if presentation_only:
            # Field repair is not an electrical authoring operation. Keep all
            # existing wire/UUID/order bytes and fence non-presentation changes.
            if _presentation_signature(current) != _presentation_signature(updated):
                raise ValueError("Field-only transaction changed non-presentation structure")
        else:
...
def _set_presentation_field(block: str, field: str, x: float, y: float, angle: float,
                            justify: frozenset[str]) -> str:
    """Use quote-aware immediate child spans for new field presentation moves."""
    at_node = f"(at {_fmt_mm(x)} {_fmt_mm(y)} {int(round(angle))})"
    return _set_field_presentation(block, field, at_node, justify)
...
def _build_autoplace_fields_mutator(
    ...
    """
    source = sch_file.read_text(encoding="utf-8")
    initial = _presentation_model(source)
    wanted = set(references) if references is not None else None
    available = {symbol.reference for symbol in initial.symbols}
    if wanted is not None and (len(wanted) != len(references) or wanted - available):
        raise ValueError("Duplicate or missing requested field-placement reference")
    targets = [symbol.reference for symbol in initial.symbols
               if not symbol.lib_id.startswith("power:")
               and (wanted is None or symbol.reference in wanted)]
    updated: list[str] = []

    def mutator(content: str) -> str:
        if content != source:
            raise ValueError("Schematic source changed after field-placement planning")
        updated.clear()
        new_content = content
        for ref in targets:
            # Recompute after each move: prior fields become obstacles for later
            # symbols, including own non-target and all neighbour visible fields.
            model = _presentation_model(new_content)
            target = next(symbol for symbol in model.symbols if symbol.reference == ref)
            block, start, end = _find_presentation_symbol_span(new_content, ref)
            specs, placements = _plan_presentation_fields(target, _presentation_obstacles(model, target))
            new_block = block
            for spec, placement in zip(specs, placements, strict=True):
                if not spec.text:
                    continue
                new_block = _set_presentation_field(
                    new_block, spec.name, placement.x, placement.y, placement.angle, placement.justify
                )
            if new_block != block:
                new_content = new_content[:start] + new_block + new_content[end:]
                updated.append(ref)
        if _presentation_signature(content) != _presentation_signature(new_content):
            raise ValueError("Field repair changed non-presentation structure")
        return new_content
...
def _autoplace_fields_apply(sch_file: Path, references: list[str] | None = None) -> list[str]:
    ...
    if not targets:
        return []
    _transactional_write_to_schematic_file(sch_file, mutator, presentation_only=True)
    return updated
...
def _register_authoring(mcp: FastMCP) -> None:
    ...
    layout_automation_service = SchematicLayoutAutomationService(
        ...
        transactional_write_to_schematic_file=lambda path, mutate: _transactional_write_to_schematic_file(
            path, mutate, presentation_only=True
        ),

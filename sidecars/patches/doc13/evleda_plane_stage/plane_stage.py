...
"""
from .compact_receipt import compact_plane_stage
import copy
import json
import os
import threading
...
def bind_plane_stage(binding, ports, *, monotonic=time.monotonic, sleep=time.sleep):
    ...
    def stage(request):
        ...
        receipt = compact_plane_stage(receipt)
        if len(json.dumps(receipt,ensure_ascii=False,separators=(',', ':'),allow_nan=False).encode('utf-8'))>MAX_PAYLOAD:

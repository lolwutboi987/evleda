...
not evidence approval. DOC17 permits 1M logical nodes while retaining
the 8 MiB artifact, 500k compact-node and depth-64 bounds.
"""
import copy
import json
...
MAX_LOGICAL_NODES = 1_000_000
MAX_WIRE_NODES = 500_000
...
def compact_plane_stage(receipt):
    ...
    nodes = 0
    def bounded(value, depth=0, maximum=MAX_LOGICAL_NODES):
        nonlocal nodes
        nodes += 1
        if nodes > maximum or depth > 64:
            raise ValueError("Logical stage traversal budget exceeded")
        if isinstance(value, dict):
            for child in value.values():
                bounded(child, depth + 1, maximum)
    ...
    for call in result["rpc"]:
        ...
        for index, item in enumerate(response.get("items", [])):
            if not isinstance(item, dict) or item.get("@type") != PAD_TYPE:
                continue
    ...
    nodes = 0
    bounded(result, maximum=MAX_WIRE_NODES)

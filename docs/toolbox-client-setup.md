...
On this destination, the global `evleda_workspace` entry now uses catalog profile02 and the validated updated `evleda-pcb` skill. The exact installed STDIO command passed four read-only discovery calls, and a fresh Codex CLI app-server loaded all 15 initial tools from the real entry, including `evleda_search_library`. **Activation in the running desktop and dynamic tool refresh remain unverified.** See [current installation and preserved profile03 history](destination-client-installation.md); the repository examples remain disabled.

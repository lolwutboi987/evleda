...
The implemented [stock-catalog mode](toolbox-stock-catalog.md) adds optional `evleda_search_library` under host-approved namespace roots, followed by exact inspection and selected-source pins in compiled bundles. Public03 and a separate built-command STDIO preflight exercise 15 initial tools without opening CAD or creating a project; focused tests, source/UI typecheck and build passed separately. **That catalog profile is not the installed global profile03.** Host profile selection, desktop activation and a catalog-backed native workflow remain separate steps; the old exact-ID profile and disabled examples retain their current behavior.

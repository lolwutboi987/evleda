...
Source-only package for the demonstrated headless helper. It preserves original KiCad 10.0.3 sources and builds explicitly patched coupled-stripline and coupled-microstrip copies. It is not a new field solver, does not operate KiCad editors, and does not establish manufactured controlled-impedance compliance.
...
`main.cpp` protocol-4 wrapper SHA256 is `B9C63898E2BE7449B717908A46AD3285E03390623DE37D90792882879A1DE6C9`. `wrapper-provenance.json` records implementation revision `evleda-uncovered-coupled-microstrip-v1`, retaining the protocol-3 single-microstrip extension and adding an explicit coupled absent-cover branch. All fifteen upstream source files remain unchanged; the coupled addition is recorded in a separate patched copy. No executable, compiler archive or compiler cache is included in this package. The historical protocol-2 host executable at `D:/EvlEDA-transmission-line-core-v3-20260909/transline-core.exe` is SHA256 `EA24EFD0A7C1582A54928B33DD5710293674170A20A3E10BDE010038EB15BDBB`; it is not a protocol-4 artifact or a promise of bit-identical output across compilers. Historical helpers and evidence remain unchanged.

## Audited vendor deviation

Retained calculation correction set: `evleda-stripline-corrections-v1`. `patches.json` records the original and patched file hashes. The current wrapper retains these corrections. The build uses `patched/coupled_stripline.cpp` and `patched/coupled_microstrip.cpp` instead of their original counterparts, after verifying both SHA256 values. Original source bytes and `source-hashes.json` remain unchanged.
...
Unmodified file SHA256: `A35C802BDD7289792828D11DE32339AC9F149BBE2AA6E601566DA3613C773745`. Intermediate centered-only file SHA256: `633D22D8284B24A94868010B699E4839E538C0CB3086D144CD0978C0BA78507B`. Final two-correction file SHA256: `683FA25D85758C1335EABFCEB2B4F181A8E491E8BF04D26D8480C74F12F9DF9C`. Both retained stripline deviations are scoped to coupled stripline. The separate coupled-microstrip absent-cover patch is described below; the single-line implementations are unchanged.
...
### Explicit absent cover for single and coupled microstrip

For `microstrip` and `coupled_microstrip`, `H_T=absent` (API `parameters.H_T: "absent"`) selects an uncovered conductor instead of a finite metallic enclosure. Finite positive numeric `H_T` retains its previous meaning. Omitted values, `Infinity`, `NaN`, other strings, and absent cover on any other model are rejected. The explicit input is echoed as `H_T: {value:"absent",unit:"1"}`; no nonfinite JSON number or fabricated cover dimension is emitted.
...
For `coupled_microstrip`, the copied `patched/coupled_microstrip.cpp` uses the infinite internal `H_T` parameter only as an explicit topology sentinel. It sets even/odd cover filling multipliers to 1, skips both cover impedance corrections with 0, and supplies the auxiliary zero-thickness single line with its exact uncovered branch. The finite-cover functions are never evaluated for an absent cover. The sentinel check uses `H_T` itself, so overflow of a finite numeric `H_T/H` ratio cannot accidentally select the new topology. Finite numeric inputs preserve the original expressions and the historical auxiliary `H_T=1e12` assignment. Thickness, coupling, dispersion, losses, phase and synthesis mechanics are otherwise retained.

The distinction is necessary: in the even finite-cover fit, `E = 0.725*sin((pi/2)*10^(0.0492*g-0.073))`, where `g=S/H`. The first zero is `(log10(2)+0.073)/0.0492 = 7.602235684...`. For nonsingular `g` where `E<0`, including `g=10`, the finite-cover correction tends to 540 ohm before dielectric scaling rather than zero. Finite-cover coefficients also contain poles and branch thresholds; absent cover bypasses those coefficients entirely. The [official KiCad implementation](https://docs.kicad.org/doxygen/coupled__microstrip_8cpp_source.html) and the preserved pinned source make those terms auditable. A large finite height or evaluating the coupled fit at infinity would not implement this topology.

The [Qucs coupled-microstrip technical reference](https://qucs.sourceforge.net/tech/node77.html) documents the underlying uncovered equations and their envelope: `0.1 <= W/H <= 10`, `0.1 <= S/H <= 10`, `1 <= EPSILONR <= 18`, with impedance dispersion through `f[GHz]*H[mm] <= 20` (effective-permittivity dispersion is documented separately through 25). Its finite-thickness correction assumes `S >> 2*T`; no numeric threshold for that assumption is supplied. The adapter exposes the existing range warning and an explicit `COUPLED_MICROSTRIP_UNCOVERED_MODEL` warning including this thickness assumption. Positive-input validation and membership in that envelope are not a finite-thickness accuracy guarantee, solder-mask model, manufacturing tolerance or board qualification.

The retained coupled dielectric-loss expressions divide by `EPSILONR-1`. At exact `EPSILONR=1` the observed native loss results are nonfinite and the retained even-mode impedance calculation also returns an error, even though the impedance approximation's published range includes 1. Those calculations return exit 2 / `invalid_model_result`; the absent-cover branch does not change the loss formulas or suppress their errors.
...
Calculation stdout is one JSON object with `schemaVersion:4`, `implementationRevision:"evleda-uncovered-coupled-microstrip-v1"`, `sourceCommit`, `model`, `operation`, `converged`, `valid`, `inputs` and `results`. Inputs map parameter names to `{value,unit}`, including the explicit absent marker above; results map names to `{value,status,unit}`. Status is `ok`, `warning` or `error`. Nonfinite results are encoded as null/error. Results include analyzed or synthesized PHYS_WIDTH and coupled PHYS_S. Input exceptions produce only stderr and exit 1.

The adapter continues to accept exactly protocol 2 with revision `evleda-stripline-corrections-v1` for all-numeric requests, and protocol 3 with revision `evleda-uncovered-microstrip-v1` for numeric requests plus single-microstrip absent cover. Coupled absent cover requires protocol 4 with revision `evleda-uncovered-coupled-microstrip-v1` and its actual executable pin. A protocol-2 or protocol-3 profile does not gain coupled absent-cover capability merely by updating the TypeScript schema. Crafted absent input echoes from an older protocol are rejected. Prepare and verify a new helper/profile binding separately; do not replace an active helper or silently rewrite a profile.
...
The separate destination MSVC protocol-4 helper built on 2026-09-10 is 384000 bytes with SHA256 `B6398EF3CD9C4FFAF95873C2C5353E157A4EB8B3C292580A5DA69D782B6A2430`. Its build log, compiler/source pins and numerical evidence remain outside the repository in the new `working-helpers/transline-core-uncovered-coupled-v1-msvc-20260910` directory. Existing helper binaries were preserved. The source adapter/MCP suite passes 118 focused tests and the repository TypeScript no-emit check passes; no full distribution build or active profile replacement was performed for this package change.

Native qualification replayed 20 historical finite-cover successes and three historical single-uncovered successes with bit-identical numerical results, then exercised 360 coupled-uncovered analyses across spacing/coefficient boundaries, three widths, three permittivities and two normalized frequencies. The 240 cases with permittivity 4.2 or 18 were valid; all 120 exact-air cases returned the retained nonfinite-loss failure above. Finite controls retained the previous numerical behavior, including invalid results from the problematic cover fit. Both synthesis constraints, multiple seeds and the `S/H=10` inverse checks succeeded; synthesized differential targets are checked against `2*Z0_O`, not native quasistatic `Z_DIFF`. The final `qualification/results.corrected.json` rechecks all 553 saved subprocess outputs with 4267 passing assertions; the initial aggregate is preserved with the correction of an assertion that confused native quasistatic `Z_DIFF` with frequency-dependent `2*Z0_O`. There were no subprocess timeouts, and the worst tested inverse/target odd-mode residual was below 9.4e-7 ohm. Five probes outside the main geometry range or thickness assumption still returned native `valid=true`, demonstrating why arithmetic validity is not an applicability guard. These tests qualify the wrapper's bounded calculation behavior and branch selection, not board impedance, manufacturing suitability or absolute model accuracy.

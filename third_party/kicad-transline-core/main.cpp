...
int main(int argc,char** argv){try{
 std::string model,operation,fix; std::map<std::string,double> inputs;
 for(int i=1;i<argc;++i){std::string s=argv[i]; if(s=="--model"||s=="--operation"||s=="--fix"){
  ...
  auto key=s.substr(0,p);size_t used=0;double value=std::stod(s.substr(p+1),&used);
 ...
 for(const auto& n:required)if(!inputs.count(n))throw std::runtime_error("Missing required parameter: "+n);
 ...
 std::cout<<"{\"schemaVersion\":2,\"implementationRevision\":\"evleda-stripline-corrections-v1\",\"sourceCommit\":\"146a4f2a7585c65bc580427a19b6fe2ec4a3f622\",\"model\":\""<<model<<"\",\"operation\":\""<<operation<<"\",\"converged\":"<<(converged?"true":"false")<<",\"valid\":"<<(valid?"true":"false")<<",\"inputs\":{";
